...
contract Scheduler is SchedulerBase {
    ...
    function removeCall (bytes32 _jobID, uint256 _index) external Auth (_jobID, true) {
        ...
        uint256 topIndex = IDToJob[_jobID].calls.length - 1;

        // If the index to remove is not the last one, swap it with the last one
        if (_index != topIndex) {
            IDToJob[_jobID].calls[_index] = IDToJob[_jobID].calls[topIndex];
        }
